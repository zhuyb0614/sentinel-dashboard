...
import com.alibaba.csp.sentinel.dashboard.service.BatchRuleService;
...
public class AppInfo {
    ...
    private List<BatchRuleService> batchRuleServices;
    ...
    public boolean addMachine(MachineInfo machineInfo) {
        publishNewOrDeadMachineRules(machineInfo);
     ...
     * @param machineInfo
     */
    private void publishNewOrDeadMachineRules(MachineInfo machineInfo) {
        boolean isNewMachine = true;
        boolean isDeadMachine = true;
        for (MachineInfo oldMachineInfo : machines) {
            boolean isSameMachine = oldMachineInfo.getIp().equals(machineInfo.getIp()) && oldMachineInfo.getPort().equals(machineInfo.getPort());
            if (isSameMachine) {
                isNewMachine = false;
                isDeadMachine = !oldMachineInfo.isHealthy();
            }

        }
        if (isNewMachine || isDeadMachine) {
            boolean finalIsNewMachine = isNewMachine;
            batchRuleServices.forEach(batchRuleService -> batchRuleService.publishRules(machineInfo, finalIsNewMachine));
        }
    }
    ...
    public void setBatchRuleServices(List<BatchRuleService> batchRuleServices) {
        this.batchRuleServices = batchRuleServices;
    }
}
