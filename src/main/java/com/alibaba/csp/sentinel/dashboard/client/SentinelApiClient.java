...
public class SentinelApiClient {
    ...
    private CompletableFuture<String> executeCommand(String app, String ip, int port, String api, Map<String, String> params, boolean useHttpPost) {
        return executeCommand(app, ip, port, api, params, true, false);
    }

    private CompletableFuture<String> executeCommand(String app, String ip, int port, String api, Map<String, String> params, boolean useHttpPost, boolean mustPost) {
        ...
        boolean supportPost = StringUtil.isNotEmpty(app) && Optional.ofNullable(appManagement.getDetailApp(app))
                ...
                .orElse(false);
        if ((!useHttpPost || !supportPost) && !mustPost) {
    ...
    public CompletableFuture<Void> setParamFlowRuleOfMachine(String app, String ip, int port, List<ParamFlowRuleEntity> rules) {
        ...
        try {
            String data = JSON.toJSONString(
                    rules.stream().filter(Objects::nonNull).map(ParamFlowRuleEntity::toRule).collect(Collectors.toList())
            );
            Map<String, String> params = new HashMap<>(1);
            params.put("data", data);
    ...
    public CompletableFuture<String> modifyClusterServerFlowRules(String ip, int port, String app, List<FlowRule> clusterRules) {
        Map<String, String> params = new HashMap<>();
        params.put("data", JSON.toJSONString(clusterRules));
        params.put("namespace", app);
        return executeCommand(app, ip, port, "cluster/server/modifyFlowRules", params, true, true);
    }

    public CompletableFuture<String> modifyClusterServerParamFlowRules(String ip, int port, String app, List<ParamFlowRule> clusterRules) {
        Map<String, String> params = new HashMap<>();
        params.put("data", JSON.toJSONString(clusterRules));
        params.put("namespace", app);
        return executeCommand(app, ip, port, "cluster/server/modifyParamRules", params, true, true);
    }
}
